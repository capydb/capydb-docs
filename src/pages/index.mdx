...
Welcome to **CapybaraDB**! This guide will help you get started quickly with our powerful API service. Whether you're inserting your first document or exploring advanced features, you'll find everything you need to make your first API request in just a few simple steps.
...
```python
CAPYBARADB_API_KEY = "your_api_key_here"
...
- The `filter` parameter is a dictionary that allows you to narrow down the documents to be queried. CapybaraDB supports all filter types that are supported by MongoDB, using the same format. When a filter is provided, the query will target only the documents that match the filter criteria.
...
## Step 4: Use Your API Key and Collection URL in TypeScript

To interact with CapybaraDB's API, you'll need a **collection URL** and your **API Key** for authorization.

### Setting the API Key and Collection URL in TypeScript

For this quick start guide (non-production environment), directly assign your API key and collection URL to variables:

```typescript
const CAPYBARADB_API_KEY = "your_api_key_here";
...
- Experiment with more complex data types using CapybaraDB extended JSON.
